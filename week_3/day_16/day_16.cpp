...
const std::unordered_map<int, std::function<long long(std::vector<long long>&)>> operators = {
   {0, [](auto& vec){ return  std::accumulate (vec.begin(), vec.end(), 0ll); }                   }, // plus
   {1, [](auto& vec){ return  std::accumulate (vec.begin(), vec.end(), 1ll,std::multiplies()); } }, // multiply
...
long long read_packet(const std::string binary, int& version_sum, size_t& read_pos);
...
int main(){
   ...
   int part_1 = 0;
   long long part_2 = read_packet(binary,part_1, start_pos);
   ...
   switch (type_ID){
      ...
      default:{
         ...
         if (binary[read_pos] == '0'){
            ...
            size_t read_length = read_pos + std::stoull(binary.substr(read_pos+1,15), nullptr, 2) + 16;
            read_pos += 16;
