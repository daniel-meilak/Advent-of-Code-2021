...
void play2(std::unordered_map<game_t,ulong,hash>& all_games, ulong& wins1, ulong& wins2);
...
int main(){
    ...
    std::unordered_map<game_t,ulong,hash> all_games;
...
void play2(std::unordered_map<game_t,ulong,hash>& all_games, ulong& wins1, ulong& wins2){
    ...
    // next set of games
    std::unordered_map<game_t,ulong,hash> next_games;
    ...
    for (const auto& [game,identicals] : all_games){
        for (int i=3; i<=9; i++){
            ...
            copy.who = !copy.who;
            ulong new_identicals = identicals*outcomes.at(i);
